...
class IdentityFullTrainer(IdentityTrainer):
    def __init__(self, config: TrainingConfig, models: Dict[str, nn.Module], optimizer: torch.optim.Optimizer, criterion=None):
        super().__init__(config, models, optimizer)
    ...
    def _prepare_batch(self, batch_dict):
        ...
        return {
            ...
            self.config.batch_mel_mask_key: mel_mask,
        }
    
    def train_step(self, batch_dict):
        batch_dict = self._prepare_batch(batch_dict)
        return super().train_step(batch_dict)

    @torch.no_grad()
    def evaluate_sanity(self, batch_dict):
        batch_dict = self._prepare_batch(batch_dict)
        return super().evaluate_sanity(batch_dict)




# class IdentityFullTrainer(BaseTrainer):
#     def __init__(self, config: TrainingConfig, models: Dict[str, nn.Module], optimizer: torch.optim.Optimizer, criterion=None):
#         super().__init__(config, models, optimizer)
#         self.criterion = criterion or nn.L1Loss(reduction='none')
#         self.pooling = self.models[config.models_pooling_name]
...
#         self.feature_extractor = HubertWrapper(self.config)
    ...
    #     waves = batch_dict[self.config.batch_wave_key]
    #     srs = batch_dict[self.config.batch_sr_key]
    #     return self.feature_extractor(waves, srs)
    ...
    #     waves = batch_dict[self.config.batch_wave_key]
    #     srs = batch_dict[self.config.batch_sr_key]
    #     return self.mel_processor(waves, srs)
    ...
    #     feats, feats_mask = self._extract_hubert(batch_dict)
    #     # print(feats.shape, feats_mask.shape)
